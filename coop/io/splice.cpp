...
static int SpliceToFileImpl(Descriptor& in, int fileFd, off_t offset, int pipefd[2],
                            size_t len, bool killAware)
{
    SPDLOG_TRACE("splice-to-file in={} file={} off={} len={}", in.m_fd, fileFd, offset, len);

    // Phase 1: move data from input socket into pipe (cooperative wait on EAGAIN)
    //
    ssize_t n;
    for (;;)
    {
        ...
        spdlog::warn("splice-to-file in={} errno={}", in.m_fd, errno);
        return -errno;
    }

    // Phase 2: drain pipe into the file at offset. The kernel advances `off` per short
    // write; regular-file splice completes against the page cache so there is no readiness
    // to wait for — short writes just loop.
    //
    off_t off = offset;
    size_t remaining = (size_t)n;
    while (remaining > 0)
    {
        ssize_t w = ::splice(pipefd[0], nullptr, fileFd, &off, remaining, SPLICE_F_MOVE);
        if (w > 0)
        {
            remaining -= w;
            continue;
        ...
        spdlog::warn("splice-to-file file={} errno={}", fileFd, errno);
        return -errno;
    }
...
{
    return SpliceToFileImpl(in, fileFd, offset, pipefd, len, false);
}

int SpliceToFileKill(Descriptor& in, int fileFd, off_t offset, int pipefd[2], size_t len)
{
    return SpliceToFileImpl(in, fileFd, offset, pipefd, len, true);
}
