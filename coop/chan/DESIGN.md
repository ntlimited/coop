...
| Type | Role |
|------|------|
| `Channel<T>` / `FixedChannel<T,N>` | Intra-cooperator buffered channel. Two coordinators encode state: `m_recv` held ↔ empty, `m_send` held ↔ full. `Recv`/`Send` are kill-oblivious (cleanup); `RecvKill`/`SendKill` are the Group-cancellation siblings. `RecvChannel<T>` / `SendChannel<T>` are pointer-sized directional views (Go `<-chan` / `chan<-`); Channel::Recv/Send are direct, no virtual base. |
